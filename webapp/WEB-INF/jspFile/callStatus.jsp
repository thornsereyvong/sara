...
app.controller('campController',['SweetAlert','$scope','$http',function(SweetAlert, $scope, $http){
	...
	$scope.listStatusID = function(id){
			$http.get("${pageContext.request.contextPath}/call_status/list/"+id).success(function(response){
	...
	$scope.deleteStat = function(leadID){
        ...
        function(isConfirm){ //Function that triggers on user action.
            if(isConfirm){
	            $http.delete("${pageContext.request.contextPath}/call_status/remove/"+leadID)
	            .success(function(){
	            		SweetAlert.swal({
			            		title:"Deleted",
...
$(document).ready(function(){
	...
	$('#form_status').bootstrapValidator({
		...
		if($("#btn_save").val() == "Save"){
			$.ajax({
				url : "${pageContext.request.contextPath}/call_status/add",
				type : "POST",
				data : JSON.stringify({ 
				      "callStatusName": $("#name").val(),
				      "callStatusDes": $("#desc").val()
					}),
			...
			$.ajax({
				url : "${pageContext.request.contextPath}/call_status/edit",
				type : "PUT",
				data : JSON.stringify({
					  "callStatusId": $("#id").val(), 
				      "callStatusName": $("#name").val(),
...
<div class="content-wrapper" ng-app="campaign" id="campController" ng-controller="campController">
	...
	<section class="content-header">
		<h1>Call Status</h1>
		<ol class="breadcrumb">
			<li><a href="#"><i class="fa fa-dashboard"></i>Call Status</a></li>
		</ol>
	</section>
		...
		<div class="box">
			<div class="box-header with-border">
				...
				<form id="form_status">
				 ...
				 <div class="col-sm-12">
							...
							<input type="hidden" id="id" name="id" value="{{statusID.callStatusId}}" ng-model="statusID.callStatusId">
						...
						<div class="col-sm-10">
							<div class="form-group" id="c_name">
								<input type="text" class="form-control" name="name" id="name" ng-model="statusID.callStatusName">
							</div>
						</div>
				 		<div class="col-sm-2">
						...
						<div class="col-sm-10">
							<div class="form-group">
								<textarea style="height: 120px" rows="" cols="" name="desc" id="desc" ng-model="statusID.callStatusDes" class="form-control"></textarea>
							</div>
						</div>
			...
			<div class="tablecontainer table-responsive" data-ng-init="listStatus()" > 
				...
				<table class="table table-hover" >
						<tr>
						<th style="cursor: pointer;" ng-click="sort('callStatusId')">Status ID
								<span class="glyphicon sort-icon" ng-show="sortKey=='callStatusId'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}">
							</th>
							<th style="cursor: pointer;" ng-click="sort('callStatusName')">Status Name
							...
							<th style="cursor: pointer;" ng-click="sort('callStatusDes')">Description
								<span class="glyphicon sort-icon" ng-show="sortKey=='callStatusDes'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}">
							</th>
						...
						<tr dir-paginate="cc in status |orderBy:sortKey:reverse |filter:search |itemsPerPage:5">
							<td>{{cc.callStatusId}}</td>
							<td>{{cc.callStatusName}}</td>
							<td>{{cc.callStatusDes}}</td>
							
							<td>
								<a ng-click="listStatusID(cc.callStatusId)" class="btn btn-success custom-width"><i class="fa fa-pencil" aria-hidden="true"></i></a>
								<button type="button" ng-click="deleteStat(cc.callStatusId)" class="btn btn-danger custom-width"><i class="fa fa-times" aria-hidden="true"></i></button>
							</td>
						</tr>
