...
app.controller('campController',['SweetAlert','$scope','$http',function(SweetAlert, $scope, $http){
	...
	$scope.listStatus = function(){
		$http.get("${pageContext.request.contextPath}/case_type/list").success(function(response){
	...
	$scope.listStatusID = function(id){
			$http.get("${pageContext.request.contextPath}/case_type/list/"+id).success(function(response){
	...
	$scope.deleteStat = function(leadID){
        ...
        function(isConfirm){ //Function that triggers on user action.
            if(isConfirm){
	            $http.delete("${pageContext.request.contextPath}/case_type/remove/"+leadID)
	            .success(function(){
	            		SweetAlert.swal({
			            		title:"Deleted",
...
$(document).ready(function(){
	...
	$('#form_status').bootstrapValidator({
		...
		if($("#btn_save").val() == "Save"){
			$.ajax({
				url : "${pageContext.request.contextPath}/case_type/add",
				type : "POST",
				data : JSON.stringify({ 
				      "caseTypeName": $("#name").val(),
				      "caseTypeDes": $("#desc").val()
					}),
			...
			$.ajax({
				url : "${pageContext.request.contextPath}/case_type/edit",
				type : "PUT",
				data : JSON.stringify({
					  "caseTypeId": $("#id").val(), 
				      "caseTypeName": $("#name").val(),
...
<div class="content-wrapper" ng-app="campaign" id="campController" ng-controller="campController">
	...
	<section class="content-header">
		<h1>Case Types</h1>
		<ol class="breadcrumb">
			<li><a href="#"><i class="fa fa-dashboard"></i>Case Types</a></li>
		</ol>
	</section>
		...
		<div class="box">
			<div class="box-header with-border">
				...
				<form id="form_status">
				 ...
				 <div class="col-sm-12">
							...
							<input type="hidden" id="id" name="id" value="{{statusID.caseTypeId}}" ng-model="statusID.caseTypeId">
						...
						<div class="col-sm-10">
							<div class="form-group" id="c_name">
								<input type="text" class="form-control" name="name" id="name" ng-model="statusID.caseTypeName">
							</div>
						</div>
				 		<div class="col-sm-2">
						...
						<div class="col-sm-10">
							<div class="form-group">
								<textarea style="height: 120px" rows="" cols="" name="desc" id="desc" ng-model="statusID.caseTypeDes" class="form-control"></textarea>
							</div>
						</div>
			...
			<div class="tablecontainer table-responsive" data-ng-init="listStatus()" > 
				...
				<table class="table table-hover" >
						<tr>
						<th style="cursor: pointer;" ng-click="sort('caseTypeId')">Type ID
								<span class="glyphicon sort-icon" ng-show="sortKey=='caseTypeId'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}">
							</th>
							<th style="cursor: pointer;" ng-click="sort('caseTypeName')">Type Name
							...
							<th style="cursor: pointer;" ng-click="sort('caseTypeDes')">Description
								<span class="glyphicon sort-icon" ng-show="sortKey=='caseTypeDes'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}">
							</th>
						...
						<tr dir-paginate="cc in status |orderBy:sortKey:reverse |filter:search |itemsPerPage:5">
							...
							<td>{{cc.caseTypeDes}}</td>
							
							<td>
								<a ng-click="listStatusID(cc.caseTypeId)" class="btn btn-success custom-width"><i class="fa fa-pencil" aria-hidden="true"></i></a>
								<button type="button" ng-click="deleteStat(cc.caseTypeId)" class="btn btn-danger custom-width"><i class="fa fa-times" aria-hidden="true"></i></button>
							</td>
						</tr>
