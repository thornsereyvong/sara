...
app.controller('campController',['SweetAlert','$scope','$http',function(SweetAlert, $scope, $http){
	...
	$scope.listStatus = function(){
		$http.get("${pageContext.request.contextPath}/industry/list").success(function(response){
	...
	$scope.listStatusID = function(id){
			$http.get("${pageContext.request.contextPath}/industry/list/"+id).success(function(response){
					$scope.statusID = response.RESPONSE_DATA;
					$scope.action = "Update";
				});
			};
	...
	$scope.deleteStat = function(leadID){
		SweetAlert.swal({
            ...
            text: "This industry will not be able to recover!", //light text
        ...
        function(isConfirm){ //Function that triggers on user action.
            if(isConfirm){
	            $http.delete("${pageContext.request.contextPath}/industry/remove/"+leadID)
	            .success(function(){
	            		SweetAlert.swal({
			            		title:"Deleted",
			            		text:"industry have been deleted!",
			            		type:"success",  
			            		timer: 2000,   
...
$(document).ready(function(){
	...
	$('#form_status').bootstrapValidator({
		...
		if($("#btn_save").val() == "Save"){
			$.ajax({
				url : "${pageContext.request.contextPath}/industry/add",
				type : "POST",
				data : JSON.stringify({ 
				      "industName": $("#name").val(),
				...
				success:function(data){
						...
						swal({
		            		title:"Success",
		            		text:"User have been created new industry!",
		            		type:"success",  
		            		timer: 2000,   
		            		showConfirmButton: false
	        			});
			...
			$.ajax({
				url : "${pageContext.request.contextPath}/industry/edit",
				type : "PUT",
				data : JSON.stringify({
					  "industID": $("#id").val(), 
				      "industName": $("#name").val(),
				...
				success:function(data){
						...
						swal({
		            		title:"Success",
		            		text:"User have been Updated industry!",
		            		type:"success",  
		            		timer: 2000,   
		            		showConfirmButton: false
	        			});
		...
		<div class="box">
			<div class="box-header with-border">
				...
				<form id="form_status">
				 ...
				 <div class="col-sm-12">
							...
							<input type="hidden" id="id" name="id" value="{{statusID.industID}}" ng-model="statusID.industID">
						
						 <div class="col-sm-2">
							<label class="font-label">Source Name :</label>
						</div>
						<div class="col-sm-10">
							<div class="form-group" id="c_name">
								<input type="text" class="form-control" name="name" id="name" ng-model="statusID.industName">
							</div>
						</div>
				 		<div class="col-sm-2">
			...
			<div class="tablecontainer table-responsive" data-ng-init="listStatus()" > 
				...
				<table class="table table-hover" >
						<tr>
						<th style="cursor: pointer;" ng-click="sort('industID')">Industry ID
								<span class="glyphicon sort-icon" ng-show="sortKey=='industID'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}">
							</th>
							<th style="cursor: pointer;" ng-click="sort('industName')">Industry Name
								<span class="glyphicon sort-icon" ng-show="sortKey=='industName'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}">
							</th>
						...
						<tr dir-paginate="cc in status |orderBy:sortKey:reverse |filter:search |itemsPerPage:5">
							...
							<td>
								<a ng-click="listStatusID(cc.industID)" class="btn btn-success custom-width"><i class="fa fa-pencil" aria-hidden="true"></i></a>
								<button type="button" ng-click="deleteStat(cc.industID)" class="btn btn-danger custom-width"><i class="fa fa-times" aria-hidden="true"></i></button>
							</td>
						</tr>
