...
app.controller('campController',['SweetAlert','$scope','$http',function(SweetAlert, $scope, $http){
	...
	$scope.listStatus = function(){
		$http.get("${pageContext.request.contextPath}/lead_source/list").success(function(response){
	...
	$scope.listStatusID = function(id){
			$http.get("${pageContext.request.contextPath}/lead_source/list/"+id).success(function(response){
	...
	$scope.deleteStat = function(leadID){
		SweetAlert.swal({
            ...
            text: "This Source will not be able to recover!", //light text
        ...
        function(isConfirm){ //Function that triggers on user action.
            if(isConfirm){
	            $http.delete("${pageContext.request.contextPath}/lead_source/remove/"+leadID)
	            .success(function(){
	            		SweetAlert.swal({
			            		title:"Deleted",
			            		text:"Source have been deleted!",
			            		type:"success",  
			            		timer: 2000,   
...
$(document).ready(function(){
	...
	$('#form_status').bootstrapValidator({
		...
		if($("#btn_save").val() == "Save"){
			$.ajax({
				url : "${pageContext.request.contextPath}/lead_source/add",
				type : "POST",
				data : JSON.stringify({ 
				      "sourceName": $("#name").val(),
				...
				success:function(data){
						...
						swal({
		            		title:"Success",
		            		text:"User have been created new Source!",
		            		type:"success",  
		            		timer: 2000,   
		            		showConfirmButton: false
	        			});
			...
			$.ajax({
				url : "${pageContext.request.contextPath}/lead_source/edit",
				type : "PUT",
				data : JSON.stringify({
					  "sourceID": $("#id").val(), 
				      "sourceName": $("#name").val(),
				...
				success:function(data){
						...
						swal({
		            		title:"Success",
		            		text:"User have been Updated Source!",
		            		type:"success",  
		            		timer: 2000,   
		            		showConfirmButton: false
	        			});
...
<div class="content-wrapper" ng-app="campaign" id="campController" ng-controller="campController">
	...
	<section class="content-header">
		<h1>Lead Sources</h1>
		<ol class="breadcrumb">
			<li><a href="#"><i class="fa fa-dashboard"></i>Lead Sources</a></li>
		</ol>
	</section>
		...
		<div class="box">
			<div class="box-header with-border">
				...
				<form id="form_status">
				 ...
				 <div class="col-sm-12">
							...
							<input type="hidden" id="id" name="id" value="{{statusID.sourceID}}" ng-model="statusID.sourceID">
						...
						<div class="col-sm-10">
							<div class="form-group" id="c_name">
								<input type="text" class="form-control" name="name" id="name" ng-model="statusID.sourceName">
							</div>
						</div>
				 		<div class="col-sm-2">
			...
			<div class="tablecontainer table-responsive" data-ng-init="listStatus()" > 
				...
				<table class="table table-hover" >
						<tr>
						<th style="cursor: pointer;" ng-click="sort('sourceID')">Source ID
								<span class="glyphicon sort-icon" ng-show="sortKey=='sourceID'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}">
							</th>
							<th style="cursor: pointer;" ng-click="sort('sourceName')">Source Name
						...
						<tr dir-paginate="cc in status |orderBy:sortKey:reverse |filter:search |itemsPerPage:5">
							...
							<td>
								<a ng-click="listStatusID(cc.sourceID)" class="btn btn-success custom-width"><i class="fa fa-pencil" aria-hidden="true"></i></a>
								<button type="button" ng-click="deleteStat(cc.sourceID)" class="btn btn-danger custom-width"><i class="fa fa-times" aria-hidden="true"></i></button>
							</td>
						</tr>
