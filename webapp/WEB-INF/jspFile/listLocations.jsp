...
app.controller('campController',['$scope','$http',function($scope, $http){
	...
	$scope.deleteCon = function(oppID){
		...
		if(str == "YES"){
			swal({   
				title: "<span style='font-size: 25px;'>You are about to delete location with ID: <span class='color_msg'>"+oppID+"</span>.</span>",   
				...
				setTimeout(function(){			
					$.ajax({ 
			    		url: "${pageContext.request.contextPath}/event_location/remove/"+oppID,
			    		method: "DELETE",
			...
			<div class="panel panel-default">
  				<div class="panel-body">
					...
					<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
						<div class="row">
							<div class="tablecontainer table-responsive" data-ng-init="listLocations()" > 
								...
								<table class="table table-hover" >
									<tr>
										...
										<th style="cursor: pointer;" ng-click="sort('loNo')">No 
											<span class="glyphicon sort-icon" ng-show="sortKey=='loNo'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}">
										</th>
										<th style="cursor: pointer;" ng-click="sort('loStreet')">Street 
											<span class="glyphicon sort-icon" ng-show="sortKey=='loStreet'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}">
										</th>
										<th style="cursor: pointer;" ng-click="sort('loCity')">City 
											<span class="glyphicon sort-icon" ng-show="sortKey=='loCity'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}">
										</th>
										<th style="cursor: pointer;" ng-click="sort('loCountry')">Country 
											<span class="glyphicon sort-icon" ng-show="sortKey=='loCountry'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}">
										</th>				
										<th class="text-center">Action</th>
									</tr>
									<tr dir-paginate="cc in locations |orderBy:sortKey:reverse |filter:search |itemsPerPage:pageSize.row" class="ng-cloak">
										<td>{{cc.loId}}</td>
										<td>{{cc.loName}}</td>
										<td>{{cc.loNo == ''?'-':cc.loNo}}</td>
										<td>{{cc.loStreet == ''?'-':cc.loStreet}}</td>
										<td>{{cc.loCity  == ''?'-':cc.loCity}}</td>
										<td>{{cc.loCountry == ''?'-':cc.loCountry}} </td>
										<td class="text-center" style="min-width: 100px;">
											<a href="${pageContext.request.contextPath}/update-location/{{cc.loId}}"><button type="button" class="btn btn-xs" data-toggle="tooltip" title="edit"><i class="fa fa-pencil text-primary"></i></button></a>
											<a href="#" ng-click="deleteCon(cc.loId)"><button type="button" class="btn btn-xs" data-toggle="tooltip" title="delete"><i class="fa fa-trash text-danger"></i></button></a>
											<a href="${pageContext.request.contextPath}/view-location/{{cc.loId}}"><button type="button" data-toggle="tooltip" class="btn btn-xs" title="view"><i class="fa fa-eye text-info"></i></button></a>
										</td>	
									</tr>
								</table>
							    <%}else{%>
