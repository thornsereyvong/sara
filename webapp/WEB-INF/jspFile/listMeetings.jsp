...
app.controller('campController',['$scope','$http',function($scope, $http){
	...
	$scope.deleteCon = function(oppID){
		...
		if(str == "YES"){
			swal({   
				title: "<span style='font-size: 25px;'>You are about to delete meeting with ID: <span class='color_msg'>"+oppID+"</span>.</span>",   
				...
				setTimeout(function(){			
					$.ajax({ 
			    		url: "${pageContext.request.contextPath}/meeting/remove/"+oppID,
			    		method: "DELETE",
							...
							<div class="col-sm-12">
								<div class="tablecontainer table-responsive" data-ng-init="listContact()" > 
									...
									<table class="table table-hover" >
											<tr>
												<th style="cursor: pointer;" ng-click="sort('meetingId')">ID
											...
											<tr dir-paginate="cc in contact |orderBy:sortKey:reverse |filter:search |itemsPerPage:pageSize.row" class="ng-cloak">
												<td>{{cc.meetingId}}</td>
												<td>{{cc.meetingSubject.trunc(20)}} </td>
												<td ng-if="cc.meetingLocation == ''">-</td>
												<td ng-if="cc.meetingLocation != ''">{{cc.meetingLocation}}</td>
												<td style="min-width: 150px;">{{cc.meetingStartDate | date:'dd-MM-yyyy'}}</td>
												<td style="min-width: 150px;">{{cc.meetingEndDate | date:'dd-MM-yyyy'}}</td>
												<td>{{cc.statusName}}</td>	
												<td class="text-center" style="min-width: 100px;">
													<a href="${pageContext.request.contextPath}/update-meeting/{{cc.meetingId}}"><button type="button" class="btn btn-xs" data-toggle="tooltip" title="edit"><i class="fa fa-pencil text-primary"></i></button></a>
													<a href="#" ng-click="deleteCon(cc.meetingId)"><button type="button" class="btn btn-xs" data-toggle="tooltip" title="delete"><i class="fa fa-trash text-danger"></i></button></a>
													<a href="${pageContext.request.contextPath}/view-meeting/{{cc.meetingId}}"><button type="button" data-toggle="tooltip" class="btn btn-xs" title="view"><i class="fa fa-eye text-info"></i></button></a>
												</td>
											</tr>
									</table>
								    <%	}else{%>
