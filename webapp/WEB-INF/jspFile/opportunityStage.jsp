...
app.controller('campController',['SweetAlert','$scope','$http',function(SweetAlert, $scope, $http){
	...
	$scope.listStatusID = function(id){
			$http.get("${pageContext.request.contextPath}/op_stage/list/"+id).success(function(response){
...
$(document).ready(function(){
	...
	$('#form_status').bootstrapValidator({
		...
		if($("#btn_save").val() == "Save"){
			$.ajax({
				url : "${pageContext.request.contextPath}/op_stage/add",
				type : "POST",
				data : JSON.stringify({ 
				      "osName": $("#name").val(),
				      "osDes": $("#desc").val()
				...
				success:function(data){
						...
						swal({
		            		title:"Success",
		            		text:"User have been created new Stage!",
		            		type:"success",  
		            		timer: 2000,   
		            		showConfirmButton: false
	        			});
			...
			$.ajax({
				url : "${pageContext.request.contextPath}/op_stage/edit",
				type : "PUT",
				data : JSON.stringify({
					  "osId": $("#id").val(), 
				      "osName": $("#name").val(),
				...
				success:function(data){
						...
						swal({
		            		title:"Success",
		            		text:"User have been Updated Stage!",
		            		type:"success",  
		            		timer: 2000,   
		            		showConfirmButton: false
	        			});
...
<div class="content-wrapper" ng-app="campaign" id="campController" ng-controller="campController">
	...
	<section class="content-header">
		<h1>Opportunity Stage</h1>
		<ol class="breadcrumb">
			<li><a href="#"><i class="fa fa-dashboard"></i>Opportunity Stage</a></li>
		</ol>
	</section>
		...
		<div class="box">
			<div class="box-header with-border">
				...
				<form id="form_status">
				 ...
				 <div class="col-sm-12">
							...
							<input type="hidden" id="id" name="id" value="{{statusID.osId}}" ng-model="statusID.osId">
						
						 <div class="col-sm-2">
							<label class="font-label">Stage Name :</label>
						</div>
						<div class="col-sm-10">
							<div class="form-group" id="c_name">
								<input type="text" class="form-control" name="name" id="name" ng-model="statusID.osName">
							</div>
						</div>
				 		<div class="col-sm-2">
						...
						<div class="col-sm-10">
							<div class="form-group">
								<textarea style="height: 120px" rows="" cols="" name="desc" id="desc" ng-model="statusID.osDes" class="form-control"></textarea>
							</div>
						</div>
			...
			<div class="tablecontainer table-responsive" data-ng-init="listStatus()" > 
				...
				<table class="table table-hover" >
						<tr>
						<th style="cursor: pointer;" ng-click="sort('osId')">Stage ID
								<span class="glyphicon sort-icon" ng-show="sortKey=='osId'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}">
							</th>
							<th style="cursor: pointer;" ng-click="sort('osName')">Stage Name
							...
							<th style="cursor: pointer;" ng-click="sort('osDes')">Description
								<span class="glyphicon sort-icon" ng-show="sortKey=='osDes'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}">
							</th>
						...
						<tr dir-paginate="cc in status |orderBy:sortKey:reverse |filter:search |itemsPerPage:5">
							...
							<td>
								<a ng-click="listStatusID(cc.osId)" class="btn btn-success custom-width"><i class="fa fa-pencil" aria-hidden="true"></i></a>
								<button type="button" ng-click="deleteStat(cc.osId)" class="btn btn-danger custom-width"><i class="fa fa-times" aria-hidden="true"></i></button>
							</td>
						</tr>
