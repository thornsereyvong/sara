...
function listDataSalOrdById(data){
	if(data != ""){		
		if(data.postStatus != 'Open'){
			swal({   
				title: "Warning!",   
				text: "The sale order with record id: '"+data.saleId+"'  was authorized. You can not edit this sale order.",   
				type: "warning",   
				...
				window.location.replace(server+"sale-order/list");
			});
		}
...
function saleOrder(){
	...
	swal({   
		title: "<span style='font-size: 25px;'>You are about to save a sale order with id: <span style='color:#F8BB86'>"+entryNo+"</span> .</span>",   
...
function cancel(){
	document.location.href = server+"sale-order/edit/"+SalID;
}
